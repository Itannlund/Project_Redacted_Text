...
import {
    type List, pair, filter, for_each, length
} from "./libr/list";

import {
    type Queue, empty, is_empty, enqueue, dequeue, head
} from "./libr/queue_array";
...
export type Tokenized_Text = Array<string>;


export function redact_all_text(input: Array<string>): Array<string> {
...
function input() {
   const guess = prompt("guess a word")
   return guess
}
...
export function tokenize_text(t: string): Array<string>{
